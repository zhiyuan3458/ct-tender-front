      ...
      <div class="box-container nav-container">
        <div class="box-header address-header">
          <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>缴费审核</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
      </div>
...
import { paycostauditingListPayJson } from 'src/api/tender/projectmanagement';
export default {
  name: '/tender/projectmanagement/paycostauditing',
  data () {
    return {
      searchData: {
        projectName: '',
  ...
  methods: {
    ...
    async getList () {
      try {
        ...
        const { data } = await paycostauditingListPayJson(params);
        ...
        console.log(`tender-projectmanagement-paycostauditing-getList error: ${e}`);
      }
    },
    handleView (obj) {
      this.$router.push(`/tender/projectmanagement/paycostauditing/detailslist?tenderBatchInfoId=${obj.row.id}`);
    },
    handleSubmit () {
      this.getList();
    }
