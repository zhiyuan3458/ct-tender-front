    ...
    <div class="add-edit-container">
      <div class="form-container">
        <el-form :model="formData" ref="ctForm" label-width="150px">
          ...
          <el-form-item class="row-two" label="发出时间" prop="issueDate">
            <el-input maxlength="17" v-model="formData.issueDate" disabled></el-input>
          </el-form-item>
          <el-form-item class="one-row"  label="通知书内容:" prop="noticeContent">
            <el-input class="one-row" disabled type="textarea" v-model="formData.noticeContent"></el-input>
          </el-form-item>
...
import { prequalificationDetails, prequalificationAudit } from 'src/api/tender/projectmanagement';
...
export default {
  ...
  methods: {
    ...
    async getFormData (id, state) {
      ...
      this.isShow = true;
      try {
        const { data } = await prequalificationDetails({ id });
    ...
    async submitFn () {
      try {
        const { data } = await await prequalificationAudit(this.formData);
