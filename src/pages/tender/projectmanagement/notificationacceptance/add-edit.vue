...
import { winbidbulletinShowOrEdit, winbidbulletinSubmit } from 'src/api/tender/projectmanagement';
export default {
  components: {
    dictionaryComp,
    uploader
  ...
  methods: {
    ...
    async getFormData (id, state) {
      ...
      this.isShow = true;
      try {
        const { data } = await winbidbulletinShowOrEdit({ id });
    ...
    async submitFn () {
      this.$refs.ctForm.validate(async (valid) => {
        if (valid) {
          try {
            const { data } = await await winbidbulletinSubmit(this.formData);
