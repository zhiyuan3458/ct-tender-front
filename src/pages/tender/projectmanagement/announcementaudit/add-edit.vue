    ...
    <div class="add-edit-container">
      <div class="form-container">
        <el-form :model="formData" ref="ctForm" label-width="150px">
          <el-form-item label="招标人名称" prop="ownerName" class="one-row">
...
import { announcementAuditShowOrEdit, announcementAuditAudit } from 'src/api/tender/projectmanagement';
export default {
  components: {
    dictionaryComp,
    uploader
  ...
  methods: {
    ...
    async getFormData (id, state) {
      ...
      this.isShow = true;
      try {
        const { data } = await announcementAuditShowOrEdit({ id });
    ...
    async submitFn () {
      this.$refs.ctForm.validate(async (valid) => {
        if (valid) {
          try {
            const { data } = await await announcementAuditAudit(this.formData);
