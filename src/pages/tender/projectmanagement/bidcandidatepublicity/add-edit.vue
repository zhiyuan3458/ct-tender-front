    ...
    <div class="add-edit-container">
      <div class="form-container">
        <el-form :model="formData" ref="ctForm" label-width="150px">
          ...
          <el-form-item label="招标人名称" prop="ownerName">
            <el-input maxlength="17" v-model="formData.ownerName" disabled></el-input>
          </el-form-item>
          <el-form-item class="row-two" label="公示类型" prop="publicityType">
            <dictionary-comp
              v-model="formData.publicityType"
              code="publicityType"
              type="select"
              :readonly="!isShowSave"
            >
          ...
          <el-form-item label="公示开始时间" prop="publicityStartDate">
            <el-input maxlength="17" v-model="formData.publicityStartDate" disabled></el-input>
          </el-form-item>
          <el-form-item label="公示结束时间" prop="publicityEndDate">
            <el-input maxlength="17" v-model="formData.publicityEndDate" disabled></el-input>
          </el-form-item>
          <el-form-item class="one-row" label="公示内容" prop="publicityContent">
            <el-input class="one-row" disabled type="textarea" v-model="formData.publicityContent"></el-input>
          </el-form-item>
...
import { bidcandidatepublicityShowOrEdit, bidcandidatepublicityAudit } from 'src/api/tender/projectmanagement';
export default {
  components: {
    dictionaryComp,
    uploader
  ...
  methods: {
    ...
    async getFormData (id, state) {
      ...
      this.isShow = true;
      try {
        const { data } = await bidcandidatepublicityShowOrEdit({ id });
    ...
    async submitFn () {
      this.$refs.ctForm.validate(async (valid) => {
        if (valid) {
          try {
            const { data } = await await bidcandidatepublicityAudit(this.formData);
