...
import { tendernoticeListJson } from 'src/api/tender/bidderprojectinformation';
export default {
  name: '/tender/bidderprojectinformation/tendernotice',
  data () {
    return {
      searchData: {
        ownerName: '',
  ...
  methods: {
    ...
    async getList () {
      ...
      const { data } = await tendernoticeListJson(params);
      try {
