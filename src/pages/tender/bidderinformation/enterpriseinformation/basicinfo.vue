  ...
  <div class="Informationbase">
    ...
    <div class="table">
      <el-form :model="formData" ref="ctForm" label-width="200px">
        ...
        <el-row class="row-bg" v-if="formData.certificateType === '1'">
          ...
          <el-col :span="12">
            <el-form-item label="统一社会信用代码证扫描件:" prop="attachmentIdOfSocialCredit" :rules="isRequired ? [$validate.RequiredChange] : ''">
              <uploader v-model="formData.attachmentIdOfSocialCredit" :is-validate="!disabled" :disabled="disabled"></uploader>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row class="row-bg" v-if="formData.certificateType === '0'">
          <el-col :span="12">
            <el-form-item label="组织机构代码:" prop="organizationCode">
              <el-input maxlength="10" v-model="formData.organizationCode" :disabled="disabled" :rules="isRequired ? [$validate.Required] : ''"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row class="row-bg" v-if="formData.certificateType === '0'">
          <el-col :span="12">
            <el-form-item label="营业执照号:" prop="licenseCode" :rules="isRequired ? [$validate.RequiredChange] : ''">
  ...
  methods: {
    ...
    async getCompanyInfo () {
      try {
        ...
        console.log(`tender-bidderinformation-enterpriseinformation-basicinfo-getCompanyInfo error: ${e}`);
      }
    },
    /**
     * @保存企业基本信息接口
      ...
      this.$refs.ctForm.validate(async (valid) => {
        if (valid) {
          try {
            ...
            console.log(`tender-bidderinformation-enterpriseinformation-basicinfo-saveFn error： ${e}`);
          }
        } else {
          return false;
        }
    ...
    async getRegCity (provinceCode) {
      try {
        ...
        console.log(`tender-bidderinformation-enterpriseinformation-basicinfo-getRegCity error: ${e}`);
      }
    },
    /**
     * @description 初始化数据字典字段
     * @author wenruihuan
     * 2019/10/29
     */
    getRegCapitalCurrency (val) {
      this.formData.regCapitalCurrency = val ? val.displayValue : '';
    },
    /**
     * @description 获取历史信息
     * @author lizipei
     ...
     */
    getMoneyUnit (val) {
      this.formData.moneyUnit = val ? val.displayValue : '';
    },
    /**
     * @description 获取历史信息
     * @author lizipei
    ...
    async getHistoryInfo () {
      try {
        ...
        console.log(`tenderagencyinfobase-getHistoryInfo error: ${e}`);
      }
    },
    /**
      ...
      this.$refs.ctForm.validate(async (valid) => {
        if (valid) {
          try {
            ...
            console.log(`basicinfo-saveFn error: ${e}`);
          }
        } else {
          this.$refs.ctForm.clearValidate();
          this.$message({
