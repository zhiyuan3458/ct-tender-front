...
export default {
  ...
  methods: {
    ...
    async getRegCity (val) {
      try {
        const { data } = await city({ provinceCode: val });
        ...
        console.log(`tender-tenderagencyinfobase-enterpriseinformation-basicinfo-getRegCity error: ${e}`);
      }
    },
    /**
    ...
    async getCompanyInfo () {
      try {
        ...
        if (data.code === process.env.ERR_OK) {
          ...
          this.getRegCity(this.formData.regProvince);
        }
      } catch (e) {
        console.log(`tender-tenderagencyinfobase-enterpriseinformation-basicinfo-getCompanyInfo error: ${e}`);
      }
    },
    /**
     * @description 获取历史信息
    ...
    async getHistoryInfo () {
      try {
        ...
        console.log(`tender-tenderagencyinfobase-enterpriseinformation-basicinfo-getHistoryInfo error: ${e}`);
      }
    },
    /**
      ...
      this.$refs.ctForm.validate(async (valid) => {
        if (valid) {
          try {
            ...
            console.log(`tender-tenderagencyinfobase-enterpriseinformation-basicinfo-saveFn error: ${e}`);
          }
        } else {
          return false;
        }
    ...
    /**
     * @description 提交数据，与保存不一样，保存类似于保存草稿
     * @author wenruihuan
     * 2019/10/28
     */
    submitFn () {
      this.$refs.ctForm.validate(async (valid) => {
        if (valid) {
          try {
            ...
            console.log(`tender-tenderagencyinfobase-enterpriseinformation-basicinfo-submitFn error: ${e}`);
          }
        } else {
          this.clearValidate();
