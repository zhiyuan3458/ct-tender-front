...
import { tenderListJson, remove, submitToAgency } from 'src/api/tender/tendermanagement/tenderbatchinformation';

export default {
  name: '/tender/tendermanagement/tenderbatchinformation',
  data () {
    return {
      searchParams: {
        projectName: '',
  ...
  methods: {
    ...
    /**
     * @description 点击提交代理触发
     * @author lizipei
     * @date 2019/11/12
     */
    handleSubmitAgent (obj) {
      this.$confirm('是否提交给招标代理?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const { data } = await submitToAgency({ id: obj.row.id });
          if (data.code === process.env.ERR_OPERATE_OK) {
            this.getList();
          }
        } catch (e) {
          console.log(`handleSubmitAgent-error: ${e}`);
        }
      }).catch((e) => {
        console.log(e);
      });
